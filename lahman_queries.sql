...
SELECT  playerid, 
		...
		SUM(sal.salary) /SUM(pi.so) AS eff
...
WHERE pi.gs >= 10 AND yearid = 2016
GROUP BY playerid
ORDER BY eff DESC;
...
SELECT namefirst, 
		...
		CASE 
			...
			WHEN inducted = 'N' THEN NULL
			ELSE NULL END AS year_inducted
FROM total_hits
...
WITH total_hits_per_team AS(
	SELECT playerid, namefirst, namelast, bat.teamid,
			SUM(h) AS total_hits
	FROM people AS pe
	INNER JOIN batting AS bat
	USING(playerid)
	GROUP BY playerid, bat.teamid
)

SELECT namefirst, namelast, total_hits, teamid
FROM total_hits_per_team
WHERE playerid IN (
	SELECT playerid
	FROM total_hits_per_team
...
SELECT people.nameFirst, people.nameLast, MAX(Batting.HR) AS HR_2016
FROM people
JOIN Batting
ON people.playerID = Batting.playerID
WHERE Batting.yearID = 2016
AND Batting.HR = (SELECT MAX(HR) FROM Batting WHERE playerID = people.playerID)
AND EXISTS (SELECT 1 FROM Batting WHERE playerID = people.playerID AND yearID >= 2007 AND yearID <= 2016)
AND Batting.HR > 0
